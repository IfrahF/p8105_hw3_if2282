...
*Description:*
The Accelerometer data set contains `r ncol(accel)` variables and `r nrow(accel)` observations. The variables included are `r colnames(accel)`. The data set contains information on `r accel %>% pull(week) %>% max()` weeks of data for as many as `r accel %>% pull(time_min) %>% max()` activities. 
...
```{r}
accel %>%
  ggplot(aes(x = time_min, y = activity_count, group = day, color = day)) + geom_line()
```
